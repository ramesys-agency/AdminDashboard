  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-6 flex flex-col items-center justify-center min-h-[400px]">
        ...
        <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">${name.charAt(0).toUpperCase() + name.slice(1)} Module</h3>
        <p className="text-gray-500 dark:text-gray-400 text-center max-w-sm">
          This is the ${name} page for {activeBusiness}. The data table will be implemented here.
        </p>
      </div>
    </div>
  );
...
pages.forEach((p) => {
  const dir = join(appDir, p);
  if (!existsSync(dir)) mkdirSync(dir, { recursive: true });
  writeFileSync(join(dir, "page.tsx"), template(p));
});
