    ...
    if (id) {
      ...
      apiClient.get<InvoiceDetail>(`/vydhra/invoices/${id}`)
        ...
        .catch(() => {
          ...
          return apiClient.get<InvoiceDetail>(`/ramesys/invoices/${id}`).then(setInvoice);
        })
        .catch((err) => {
    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-12 print:max-w-none print:p-0 print:m-0">
      <div className="flex items-center justify-between print:hidden">
        <div className="flex items-center gap-4">
          <Link href="/invoices">
            <Button variant="ghost" size="sm" className="h-8 w-8 p-0 border border-slate-200 text-slate-500 hover:text-slate-900">
        ...
        <div className="flex gap-2">
          ...
          {!isPaid && (
            <Button size="sm" className="flex items-center gap-2 bg-slate-900 border-none transition-all hover:bg-slate-800">
      ...
      <Card className="border shadow-2xl overflow-hidden bg-white print:shadow-none print:border-none">
        ...
        <div className="p-12 border-b bg-slate-50/50 flex flex-col md:flex-row justify-between items-start gap-8 print:bg-white print:p-8">
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              <p className="text-[10px] text-muted-foreground uppercase font-black tracking-widest">Billing From</p>
              <p className="text-sm font-bold">{invoice.business.name} Inc.</p>
              <p className="text-xs text-muted-foreground">
                {isVydhra ? '123 Education Square, Learning Hub' : '456 Tech Avenue, Innovation Block'}
                <br />Mumbai, MH 400001
              </p>
            </div>
          </div>
          ...
          <div className="text-left md:text-right space-y-4">
            ...
            <div className="flex flex-col md:items-end gap-1">
              <Badge className={isPaid ? 'bg-emerald-500 hover:bg-emerald-500' : isOverdue ? 'bg-rose-500 hover:bg-rose-500' : 'bg-amber-500 hover:bg-amber-500'}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 p-12 gap-12 bg-white print:p-8 print:gap-4">
          <div className="space-y-2">
            <p className="text-[10px] text-muted-foreground uppercase font-black tracking-widest">Bill To</p>
            <p className="text-lg font-bold text-slate-900 leading-tight">{clientName}</p>
            <p className="text-sm text-slate-600">
              {!isVydhra && invoice.project?.client.company && <>{invoice.project.client.company}<br /></>}
              {clientEmail}
            </p>
          </div>
          <div className="grid grid-cols-2 gap-8 md:text-right print:gap-4">
            <div className="space-y-1">
              <p className="text-[10px] text-muted-foreground uppercase font-black tracking-widest">{isVydhra ? 'Enrollment ID' : 'Project ID'}</p>
            ...
            <div className="space-y-1">
              <p className="text-[10px] text-muted-foreground uppercase font-black tracking-widest">Due Date</p>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-slate-50 hover:bg-slate-50 print:bg-slate-100">
                  <TableHead className="px-6 py-4 font-bold text-slate-900 uppercase text-[10px] tracking-widest">Description</TableHead>
                  <TableHead className="px-6 py-4 font-bold text-slate-900 uppercase text-[10px] tracking-widest text-center">Qty</TableHead>
                  <TableHead className="px-6 py-4 font-bold text-slate-900 uppercase text-[10px] tracking-widest text-right">Unit Price</TableHead>
                  <TableHead className="px-6 py-4 font-bold text-slate-900 uppercase text-[10px] tracking-widest text-right">Amount</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                <TableRow className="border-b transition-none">
                  ...
                  <TableCell className="px-6 py-8 text-center font-bold text-slate-600 print:py-4">1</TableCell>
                  <TableCell className="px-6 py-8 text-right font-bold text-slate-600 print:py-4">₹{invoice.amount.toLocaleString()}</TableCell>
                  <TableCell className="px-6 py-8 text-right font-black text-slate-900 print:py-4">₹{invoice.amount.toLocaleString()}</TableCell>
                </TableRow>
              </TableBody>
            </Table>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-slate-50/50">
                  <TableHead className="px-6 py-4 font-bold text-[10px] uppercase tracking-widest">Ref ID</TableHead>
                    ...
                    <TableRow key={p.id} className="hover:bg-slate-50/50 border-b">
                      ...
                      <TableCell className="px-6 py-4">
                        <Badge 
                          ...
                          className={p.status === 'COMPLETED' ? 'bg-emerald-500' : 'bg-amber-500'}
                        >
                          {p.status}
                        </Badge>
                      </TableCell>
