...
export function CourseForm({ mode, courseId }: CourseFormProps) {
  ...
  useEffect(() => {
    ...
    apiClient
      .get<any>(`/vydhra/courses/${courseId}`)
      .then((course) => {
        setName(course.name ?? "");
        setDescription(course.description ?? "");
        if (course.pricing && typeof course.pricing === "object") {
          const origPrices = (course.originalPricing || {}) as Record<string, number | null>;
          const loaded = Object.entries(
            course.pricing as Record<string, number>,
          ).map(([currency, amount]) => ({
            currency,
            amount: String(amount),
            originalPrice: origPrices[currency] != null ? String(origPrices[currency]) : "",
          }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const pricingPayload = pricing
      ...
      .map((p) => ({
        ...
        originalPrice: p.originalPrice.trim() !== "" ? parseFloat(p.originalPrice) : null,
      }));
    if (pricingPayload.length === 0) {
      toast.error("At least one currency price is required.");
      return;
    }
      ...
      <div className="space-y-4">
              ...
              <div key={i} className="flex gap-2 items-center">
                ...
                <div className="relative flex-1">
                  ...
                  <Input
                    ...
                    min="0"
                    step="0.01"
                    placeholder="Slashed Price (Optional)"
        ...
        {pricing.length < SUPPORTED_CURRENCIES.length && (
          <Button
            ...
            onClick={() => {
              const usedCodes = pricing.map((p) => p.currency);
              const next = SUPPORTED_CURRENCIES.find(
                (c) => !usedCodes.includes(c.code),
              );
              if (next)
                setPricing([...pricing, { currency: next.code, amount: "", originalPrice: "" }]);
            }}
          >
            <Plus className="h-3.5 w-3.5" />
            Add currency
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            <p className="text-xs font-semibold text-gray-600">
              Thumbnail{" "}
              <span className="text-gray-400 font-normal">800×800 px</span>
            </p>
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setDetailsForm((p) => ({ ...p, level: e.target.value }))
                  }
                  className="w-full h-11 rounded-md border border-input bg-background px-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
                >
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setDetailsForm((p) => ({ ...p, duration: e.target.value }))
                  }
                  className="h-11"
                />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setDetailsForm((p) => ({ ...p, category: e.target.value }))
                  }
                  className="h-11"
                />
              </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setDetailsForm((p) => ({ ...p, subtitle: e.target.value }))
                }
                className="h-11"
              />
            </div>
            <ListFieldEditor
              ...
              onChange={(vals) =>
                setDetailsForm((p) => ({ ...p, features: vals }))
              }
            ...
            <ListFieldEditor
              ...
              onChange={(vals) =>
                setDetailsForm((p) => ({ ...p, requirements: vals }))
              }
              addLabel="Add requirement"
            />
            <p className="text-xs text-gray-400 bg-amber-50 border border-amber-100 rounded-lg px-3 py-2.5">
              For rich fields like <strong>curriculum</strong>,{" "}
              <strong>tools</strong>, and <strong>projects</strong>, switch to
              JSON mode.
            </p>
          </div>
        )}
